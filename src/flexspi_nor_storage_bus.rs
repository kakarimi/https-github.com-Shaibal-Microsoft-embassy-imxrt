...
impl BlockingNorStorageBusDriver for FlexspiNorStorageBus<Blocking> {
    fn send_command(
        ...
        // TODO: Once we enhance the Bus Trait, we will have read/write API which can use FlexSPI AHP Port for cleaner read/write syntax
                ...
                NorStorageCmdType::Read => {
                    ...
                    if let Some(mut size) = cmd.data_bytes {
                        if size > MAX_FLEXSPI_TRANSFER_SIZE {
                            loop {
                                self.read_cmd_data(MAX_FLEXSPI_TRANSFER_SIZE, buffer);
...
impl<M: Mode> FlexspiNorStorageBus<M> {
    fn setup_ip_transfer(&mut self, seq_id: u8, addr: Option<u32>, size: Option<u32>) {
        ...
        // Set Tx and Rx watermark
        self.info
            .regs
            .iptxfcr()
            .modify(|_, w| unsafe { w.txwmrk().bits(self.tx_watermark) });

        self.info
            .regs
            .iprxfcr()
            .modify(|_, w| unsafe { w.rxwmrk().bits(self.rx_watermark) });
        ...
        if let Some(size) = size {
            if size > MAX_FLEXSPI_TRANSFER_SIZE {
                self.info
                    .regs
                    .ipcr1()
                    .write(|w| unsafe { w.idatsz().bits(MAX_FLEXSPI_TRANSFER_SIZE as u16) });
            } else {
                self.info
                    .regs
                    .ipcr1()
    ...
    fn program_lut(&self, cmd: &NorStorageCmd, seq_id: usize) {
        ...
        // We are hardcoding data length to 255 as this size is ignored by the Controller since we filled
        // IDATSZ bits in register IPCR1 in setup_ip_transfer function.
        // We can set data length to any value. 255 is chosen randomly.
...
impl FlexspiNorStorageBus<Blocking> {
    ...
    fn read_cmd_data(&mut self, mut size: u32, read_data: &mut [u8]) {
        ...
        loop {
            if size < self.rx_watermark as u32 {
                ...
                // Get number of slots in the FIFO. Each slot is 4 bytes
                let num_fifo_slot = size / 4;
    ...
    fn write_cmd_data(&mut self, size: usize, write_data: &[u8]) {
        ...
        if size < self.tx_watermark as usize {
            let mut byte_cnt = 0;
            let num_fifo_slot = size / 4;
            for i in 0..num_fifo_slot {
                let mut temp = 0;
            ...
            let remaining_bytes = size - (num_fifo_slot * 4);
            ...
            let num_fifo_slot = self.tx_watermark / 4;
            let mut byte_cnt = 0;
            for i in 0..num_fifo_slot {
                let mut temp = 0;
                for j in 0..4 {
